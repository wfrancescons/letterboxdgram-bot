...
async function gridlb(ctx) {
    ...
    try {
        ...
        const lastFilms = await getLastFilmsSeen(
            letterboxd_user,
            COLUMNS * ROWS,
        );
        ...
        const exampleCommand = "/gridlb 4x1 notexts";
        const tipText =
            "💡 Tip: you can define your grid or make a collage with no text\n";
        ...
        responseExtra.entities.push(
            createEntity(
                responseMessage.indexOf(exampleCommand),
                exampleCommand.length,
                "code",
            ),
        );
